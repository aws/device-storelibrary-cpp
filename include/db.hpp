...
class StreamInterface : public std::enable_shared_from_this<StreamInterface> {
  ...
  public:
    ...
    /**
     * Read a record from the DB by its sequence number with an optional hint to the storage engine for where to start
     ...
     *
     * @param identifier an alphanumeric value to uniquely identify this iterator. Provide the same identifier later on
     ...
     * @param sequence_number sequence number of the record. When opening an existing iterator it will start from this
     * record.
     */
