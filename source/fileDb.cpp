...
namespace aws::gg {
    ...
    void FileStream::loadExistingSegments() {
        ...
        if (!_segments.empty()) {
            ...
            _current_size_bytes = std::reduce(_segments.begin(), _segments.end(), 0,
                                              [](auto a, auto &b) { return a + b.totalSizeBytes(); });
        }
    }
    ...
    void FileSegment::append(BorrowedSlice d, int64_t timestamp_ms, uint64_t sequence_number) {
        ...
        auto header = LogEntryHeader{
                ...
                .crc = static_cast<int64_t>(
                        _htonll(crc32::update(crc32::update(crc32::update(0, &ts, sizeof(int64_t)), &data_len_swap, sizeof(int32_t)),
                                             d.data(), d.size()))),
                .timestamp = ts,
